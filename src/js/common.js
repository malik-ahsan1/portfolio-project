...
function makeBlob() {
    ...
    const r = random(WIDTH * 0.12, WIDTH * 0.25);
    const points = [];
...
function animateBlobs(blobs, t) {
    ...
    for (let i = blobs.length - 1; i >= 0; i--) {
        const blob = blobs[i];
        blob.cx += Math.sin(t / 2500 + i) * 0.75;
        blob.cy += Math.cos(t / 2200 + i) * 0.72;

        for (const pt of blob.points) {
            pt.r = pt.base + Math.sin(t * pt.speed + pt.phase) * blob.r * 0.018; // faster morphing
        }
